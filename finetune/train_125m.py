"""Fine-tune Ace-2504/slm-125m-e4 (full FT) on Modal. Produces one version per run:

    modal run train_125m.py --method sft     # -> /data/checkpoints/slm-125m-sft
    modal run train_125m.py --method raft    # -> /data/checkpoints/slm-125m-raft

Both init from the e4 base checkpoint, so the two versions are directly comparable.
...
image = (
    ...
    .pip_install("torch==2.4.1", "transformers==4.46.3", "accelerate>=0.34",
                 "numpy>=1.26,<2.0")
    .env({"HF_HUB_DISABLE_IMPLICIT_TOKEN": "1",       # kill any stale implicit token
          "PYTORCH_CUDA_ALLOC_CONF": "expandable_segments:True"})   # curb fragmentation OOM
    .add_local_python_source("ft_config", "ft_data")
)


@app.function(image=image, gpu=C.M125["gpu"], volumes={"/data": vol},
...
def train(method: str, limit: int = 0, epochs: int = 0):
    ...
    model = AutoModelForCausalLM.from_pretrained(cfg["base"], torch_dtype=torch.bfloat16,
                                                 token=token).to(dev)
    # Set eos explicitly to <|eos|> so generation/eos-loss are correct regardless of config.json.
    ...
    print(f"[125m/{method}] {len(ds)} examples | dropped {ds.dropped} | trunc {ds.trunc}", flush=True)
    ...
    for epoch in range(n_epochs):
        for batch in dl:
            ...
            if accum == cfg["grad_accum"]:
                ...
                if step % 20 == 0:
                    # running summed full-loss over 20*grad_accum micro-batches -> divide by both
                    # (guide P.7: log the grad-accum-corrected loss).
                    print(f"  epoch {epoch} step {step}/{total} "
                          f"loss {running/(20*cfg['grad_accum']):.4f} "
    ...
    vol.commit()
    wall = time.time() - t0
    print(f"[125m/{method}] saved {out} | {step} steps | {wall/60:.1f} min", flush=True)
