...
**Gemma 2B · QA SFT** — answers a question from a supplied passage.
...
| Method | QLoRA — 4-bit NF4 base + rank-16 LoRA |
| Parameters | 2.6B (frozen base) |
| Trainable parameters | 20.8M LoRA (0.8%) |
| Training data | 15,000 grounded QA pairs |
| Schedule | 2 epochs · 1,875 |
| Compute | 202.0 min on A100-40GB (Modal) |
| Cost | **$3.29** total lineage |
| Judge score | **9.80 / 10** |
...
ADAPTER = "Ace-2504/gemma-2-2b-sft"
...
| Metric | Value |
|---|---|
| Judge score (0–10 rubric) | **9.80** |
| Judged correctness (0–1) | 0.964 |
| Groundedness | 97.6% |
| Fabrication ↓ | 4.2% |
| Token-F1 | 0.569 |
| n | 500 |
...
- **Schedule** — 2 epochs · 1,875, 202.0 min on A100-40GB
- Loss is **masked to the answer tokens only**, so the model is never trained to reproduce the prompt

**Cost — $3.29.** 
The base is Google's and free to build on, so only this project's own work is counted: $3.29.
...
This model has its own write-up — training details, cost breakdown and live demo — at [https://slm-gemma-qa-harman.vercel.app](https://slm-gemma-qa-harman.vercel.app).
