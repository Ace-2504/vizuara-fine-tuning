...
**Gemma 2B · RAFT** — answers from the right document among distractors, and abstains when the answer is absent.
...
| Metric | Value |
|---|---|
| Judge score (0–10 rubric) | **9.74** |
| Judged correctness (0–1) | 0.975 |
| Groundedness | 99.4% |
| Fabrication ↓ | 0.8% |
| Token-F1 | 0.145 |
| n | 500 |
...
- **Schedule** — 2 epochs · 1,250, 140.0 min on A100-40GB
- Loss is **masked to the answer tokens only**, so the model is never trained to reproduce the prompt

**Cost — $2.47.** 
The base is Google's and free to build on, so only this project's own work is counted: $2.47.
...
This model has its own write-up — training details, cost breakdown and live demo — at [https://slm-gemma-raft-harman.vercel.app](https://slm-gemma-raft-harman.vercel.app).
