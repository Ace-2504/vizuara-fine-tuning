---
license: mit
base_model: Ace-2504/slm-125m-e4
language:
  - en
...
**125M · QA SFT** — answers a question from a supplied passage, and declines when the answer is not there.
...
| | |
|---|---|
| Base model | [`Ace-2504/slm-125m-e4`](https://huggingface.co/Ace-2504/slm-125m-e4) |
...
| Metric | Value |
|---|---|
| Judge score (0–10 rubric) | **3.60** |
| Judged correctness (0–1) | 0.261 |
| Groundedness | 45.6% |
| Fabrication ↓ | 6.2% |
| Token-F1 | 0.229 |
| n | 500 |
...
- **Initialised from** [`Ace-2504/slm-125m-e4`](https://huggingface.co/Ace-2504/slm-125m-e4)
- **Method** — full fine-tune
- **Data** — 15,000 grounded QA pairs, generated from a legal/financial corpus and gated by an LLM judge for faithfulness
- **Schedule** — 3 epochs · 2,812, 9.7 min on L4
- Loss is **masked to the answer tokens only**, so the model is never trained to reproduce the prompt

**Cost — $58.52.** 
This line was **pretrained from scratch**, so the total includes the base: $57.79 of A100 time across four pretraining legs (v1 → extension → e2 → e4, 27.5 h at $2.10/h) plus $0.73 for this fine-tune. Models built on someone else's base do not carry that cost.
...
- **This checkpoint scores 3.60/10** — it is published for comparison across training stages, not because it is good. See the arena for why.
...
This model has its own write-up — training details, cost breakdown and live demo — at [https://slm-125m-qa-harman.vercel.app](https://slm-125m-qa-harman.vercel.app).
